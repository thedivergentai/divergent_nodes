...
def assemble_image_grid(
    ...
    """
    if not images:
        logger.error("❌ [GridAssembly] Cannot assemble grid: Input image list is empty. Please provide images.")
        raise ValueError("Input image list cannot be empty for grid assembly.")

    if rows <= 0 or cols <= 0:
        logger.error(f"❌ [GridAssembly] Invalid grid dimensions: rows={rows}, cols={cols}. Both must be positive integers.")
    ...
    try:
        ...
        logger.debug(f"🐛 [GridAssembly] Base image properties: H={H}, W={W}, C={C}, dtype={dtype}, device={device}")

        # Check if all images have the same shape (optional but recommended)
        if not all(img.shape == (H, W, C) for img in images):
            logger.warning(f"⚠️ [GridAssembly] Images have inconsistent shapes. Using H={H}, W={W}, C={C} from the first image. The assembled grid might look incorrect.")
            # Future enhancement: Option to resize images to fit?

    except (AttributeError, IndexError, ValueError) as e:
        logger.error(f"❌ [GridAssembly] Cannot assemble grid: Invalid image data in list. Please ensure all inputs are valid image tensors. Error: {e}", exc_info=True)
    ...
    logger.debug(f"🐛 [GridAssembly] Calculated grid dimensions: {grid_height}H x {grid_width}W")
    ...
    try:
        ...
        if C == 1 and isinstance(background_color, tuple) and len(background_color) >= 1:
            ...
            logger.debug(f"🐛 [GridAssembly] Using grayscale background value: {bg_value}")
        elif C > 1 and isinstance(background_color, (int, float)):
            bg_value = (background_color,) * C # Repeat scalar for RGB/RGBA
            logger.debug(f"🐛 [GridAssembly] Using uniform multichannel background value: {bg_value}")
        elif isinstance(background_color, tuple) and len(background_color) == C:
             bg_value = background_color # Use provided tuple directly
             logger.debug(f"🐛 [GridAssembly] Using provided tuple background value: {bg_value}")
        else:
             logger.warning(f"⚠️ [GridAssembly] Background color '{background_color}' incompatible with image channels C={C}. Defaulting to 1.0 (white).")
        ...
        logger.error(f"❌ [GridAssembly] Failed to create background grid tensor. Details: {e}", exc_info=True)
        raise RuntimeError("Failed to initialize grid tensor.") from e

    # --- Place Images onto Grid ---
    logger.info(f"🔲 [GridAssembly] Placing {len(images)} images onto {rows}x{cols} grid...")
    current_y = 0
    img_idx = 0
    for r in range(rows):
        current_x = 0
                ...
                try:
                    ...
                    if img_tensor.shape == (H, W, C):
                        grid_tensor[current_y:current_y + H, current_x:current_x + W, :] = img_tensor
                         ...
                         logger.warning(f"⚠️ [GridAssembly] Skipping image {img_idx} at grid position ({r},{c}) due to shape mismatch ({img_tensor.shape} vs {(H,W,C)}).")
                         # Leave background color in this cell
                except Exception as e:
                    logger.error(f"❌ [GridAssembly] Error placing image {img_idx} at grid position ({r},{c}). This cell will be left blank. Error: {e}", exc_info=True)
                    # Leave background color, continue to next cell
            else:
                # Handle cases where there are fewer images than grid cells
                logger.debug(f"🐛 [GridAssembly] Grid cell ({r},{c}) left blank (Image index {img_idx} >= {len(images)}).")

            current_x += W + col_gap
            img_idx += 1
        current_y += H + row_gap

    logger.info("✅ [GridAssembly] Grid assembly complete.")
...
def _get_pil_font(font_size: int = 20) -> Optional[PilFontT]:
    ...
    if os.path.exists(bundled_font_path):
        try:
            ...
            logger.debug(f"🐛 [GridAssembly] Loaded bundled font: {bundled_font_path} at size {font_size}")
            return font
        except IOError as e:
            logger.warning(f"⚠️ [GridAssembly] Failed to load bundled font '{bundled_font_path}'. Attempting system fonts. Error: {e}", exc_info=True)
    else:
        logger.debug(f"🐛 [GridAssembly] Bundled font not found at: {bundled_font_path}")
    ...
    for font_name in font_paths_to_try:
        try:
            ...
            logger.debug(f"🐛 [GridAssembly] Loaded system font: {font_name} at size {font_size}")
            return font
        except IOError:
            logger.debug(f"🐛 [GridAssembly] System font '{font_name}' not found.")
            continue

    # 3. Fallback to default PIL bitmap font
    logger.warning(f"⚠️ [GridAssembly] No TrueType fonts found. Attempting default PIL font.")
    try:
        font = ImageFont.load_default()
        logger.debug("🐛 [GridAssembly] Loaded default PIL bitmap font.")
        return font
    except IOError as e:
        logger.error(f"❌ [GridAssembly] Could not load any font for label drawing. PIL default font failed. Labels will not be drawn. Error: {e}", exc_info=True)
        return None
...
def _calculate_required_padding(draw: PilDrawT, font: PilFontT,
    ...
    try:
        ...
        logger.error(f"❌ [GridAssembly] Error calculating label padding. Labels might overlap. Details: {e}", exc_info=True)
        # Return zero padding on error to avoid breaking layout entirely
        return 0, 0

    logger.debug(f"🐛 [GridAssembly] Calculated padding: Top={top_padding}, Left={left_padding}")
...
def draw_labels_on_grid(
    ...
    """
    logger.info("✍️ [GridAssembly] Attempting to draw labels on grid...")
    if grid_tensor.dim() != 3:
         logger.error(f"❌ [GridAssembly] Cannot draw labels: Input tensor has incorrect dimensions ({grid_tensor.dim()}). Expected 3 (H, W, C).")
         return grid_tensor
    if grid_tensor.shape[2] != 3:
         logger.warning(f"⚠️ [GridAssembly] Drawing labels on non-RGB tensor (Channels={grid_tensor.shape[2]}). Color conversion might occur, which could affect label appearance.")
    ...
    try:
        ...
        if not font:
            logger.warning("⚠️ [GridAssembly] No font loaded, cannot draw labels. Returning original grid.")
            return grid_tensor

        # --- Image Conversion (Tensor -> PIL) ---
        # Ensure tensor is on CPU and in uint8 format for PIL
        logger.debug("🐛 [GridAssembly] Converting grid tensor to PIL Image...")
        try:
            ...
            if grid_pil.mode != 'RGB':
                logger.debug(f"🐛 [GridAssembly] Converting PIL image mode from {grid_pil.mode} to RGB.")
                grid_pil = grid_pil.convert('RGB')
        except Exception as e:
            logger.error(f"❌ [GridAssembly] Failed to convert grid tensor to PIL Image. Labels will not be drawn. Details: {e}", exc_info=True)
        ...
        if top_padding == 0 and left_padding == 0:
             logger.info("ℹ️ [GridAssembly] No labels provided or padding calculation failed; skipping label drawing.")
        ...
        logger.debug(f"🐛 [GridAssembly] Creating new labeled canvas: {new_width}W x {new_height}H")
        ...
        try:
            ...
            draw = ImageDraw.Draw(labeled_pil)
             ...
             logger.error(f"❌ [GridAssembly] Failed to create padded image canvas. Labels will not be drawn. Details: {e}", exc_info=True)
        ...
        if grid_rows == 0 or grid_cols == 0:
             logger.warning("⚠️ [GridAssembly] Cannot draw labels: x_labels or y_labels list is empty. Skipping label drawing.")
        ...
        logger.debug(f"🐛 [GridAssembly] Calculated cell size for label positioning (considering gaps): {H_cell}H x {W_cell}W")
        ...
        if y_axis_label: # Draw single overall label centered vertically
             ...
             logger.debug(f"🐛 [GridAssembly] Drawing Y-axis label '{y_axis_label}' at ({text_x}, {text_y})")
             ...
             for r_idx, label in enumerate(y_labels):
                  ...
                  logger.debug(f"🐛 [GridAssembly] Drawing Y label '{label}' for row {r_idx} at ({text_x}, {text_y})")
        ...
        if x_axis_label: # Draw single overall label centered horizontally
             ...
             logger.debug(f"🐛 [GridAssembly] Drawing X-axis label '{x_axis_label}' at ({text_x}, {text_y})")
             ...
             for c_idx, label in enumerate(x_labels):
                  ...
                  draw.text((text_x, text_y), label, fill=text_color, font=font)
                  logger.debug(f"🐛 [GridAssembly] Drawing X label '{label}' for col {c_idx} at ({text_x}, {text_y})")
                  # No need to track current_x_pos anymore

        # --- Convert Labeled Image Back to Tensor ---
        logger.debug("🐛 [GridAssembly] Converting labeled PIL image back to tensor...")
        labeled_np = np.array(labeled_pil).astype(np.float32) / 255.0
        final_labeled_tensor = torch.from_numpy(labeled_np)
        logger.info("✅ [GridAssembly] Label drawing complete.")
        return final_labeled_tensor # Tensor is already float32

    except Exception as e:
        logger.error(f"❌ [GridAssembly] An unexpected error occurred during label drawing process. Labels may not be displayed correctly. Details: {e}", exc_info=True)
